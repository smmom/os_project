...
// Use named semaphores for macOS compatibility
sem_t** node_sems;
int total_num_nodes;
...
void DrawGraph(Graph* graph) {
    ...
    // 2. Draw traveler paths and entities
    for (int i = 0; i < numTravelers; i++) {
        if (travelers[i].active) {
            ...
            if (travelers[i].waiting) {
                DrawText("WAIT", travelers[i].entityPosition.x - MeasureText("WAIT", 10)/2, travelers[i].entityPosition.y - ENTITY_RADIUS - 15, 10, RED);
            }
    ...
    for (int i = 0; i < numTravelers; i++) {
        ...
        if (pid == 0) { // child process
            ...
            if (dijkstra(graph, travelers[i].startNode, travelers[i].endNode, p, &len, &w)) {
                for (int j = 0; j < len; j++) {
                    // Open named semaphore for this node
                    char sem_name[32]; sprintf(sem_name, "/node_sem_%d", p[j]);
                    sem_t *sem = sem_open(sem_name, 0);

                    if (sem_trywait(sem) == -1) {
                        IPCMessage waitingMsg = { getpid(), p[j], (j < len - 1) ? p[j+1] : -1, true, false };
                        write(pipefd[1], &waitingMsg, sizeof(IPCMessage));
                        sem_wait(sem);  /* block until node is free */
                    }
...
void PauseTravelers() {
    for (int i = 0; i < numTravelers; i++) if (travelers[i].pid > 0) kill(travelers[i].pid, SIGSTOP);
}

void ResumeTravelers() {
    for (int i = 0; i < numTravelers; i++) if (travelers[i].pid > 0) kill(travelers[i].pid, SIGCONT);
}
...
int main(int argc, char* argv[]) {
    ...
    if (!file) { perror("Error opening input file"); return 1; }

    int numNodes, numEdges;
    if (fscanf(file, "%d %d", &numNodes, &numEdges) != 2) { fprintf(stderr, "Invalid graph format\n"); fclose(file); return 1; }
    if (numNodes <= 0 || numNodes > MAX_NODES) { fprintf(stderr, "Invalid number of nodes\n"); fclose(file); return 1; }

    Graph* graph = createGraph(numNodes);
    for (int i = 0; i < numEdges; i++) {
        int u, v, w;
        if (fscanf(file, "%d %d %d", &u, &v, &w) != 3) { fprintf(stderr, "Invalid edge format\n"); freeGraph(graph); fclose(file); return 1; }
        if (!addEdge(graph, u, v, w)) { fprintf(stderr, "Invalid edge values\n"); freeGraph(graph); fclose(file); return 1; }
    }

    calculateNodePositions(numNodes);
    if (fscanf(file, "%d", &numTravelers) != 1) { fprintf(stderr, "Invalid travelers count\n"); freeGraph(graph); fclose(file); return 1; }
    if (numTravelers < 0 || numTravelers > MAX_TRAVELERS) { fprintf(stderr, "Invalid number of travelers\n"); freeGraph(graph); fclose(file); return 1; }

    Color colors[] = {BLUE, GREEN, YELLOW, MAGENTA, ORANGE, LIME, SKYBLUE, VIOLET, BEIGE, BROWN};
    for (int i = 0; i < numTravelers; i++) {
        if (fscanf(file, "%d %d", &travelers[i].startNode, &travelers[i].endNode) != 2) { fprintf(stderr, "Invalid traveler format\n"); freeGraph(graph); fclose(file); return 1; }
    ...
    if (fcntl(pipefd[0], F_SETFL, O_NONBLOCK) == -1) { perror("fcntl"); close(pipefd[0]); close(pipefd[1]); freeGraph(graph); return 1; }

    // Initialize named semaphores for macOS
    total_num_nodes = numNodes;
    node_sems = malloc(sizeof(sem_t*) * total_num_nodes);
    for (int i = 0; i < total_num_nodes; i++) {
        char sem_name[32]; sprintf(sem_name, "/node_sem_%d", i);
        sem_unlink(sem_name);
        node_sems[i] = sem_open(sem_name, O_CREAT, 0644, 1);
    }
    ...
    for (int i = 0; i < total_num_nodes; i++) {
        char sem_name[32]; sprintf(sem_name, "/node_sem_%d", i);
        sem_close(node_sems[i]); sem_unlink(sem_name);
    }
    free(node_sems);
    freeGraph(graph);
    CloseWindow();
